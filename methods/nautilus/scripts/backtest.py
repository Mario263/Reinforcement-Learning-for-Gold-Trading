"""NautilusPPO backtest/evaluation — Nautilus-derived metrics for a saved policy.

  python -m methods.nautilus.scripts.backtest --model <model.zip> --split modern --device cuda
"""
import argparse
import json
...
def main():
    ...
    assert_obs(ev, cols)
    print(f"nautilus backtest {w['eval_start']}..{w['eval_end']} rows={len(ev)}")
    model = PPO.load(a.model, device=a.device)
    m = evaluate(model, ev, cols, full)
